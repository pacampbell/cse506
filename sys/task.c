...
void set_task(Task *task) {
    ...
    __asm__ __volatile__(
        "movq $0x33, %%rax;" // Load 64-bit gdt entry
        "ltr %%ax;"
...
void switch_tasks(Task *old, Task *new) {
    ...
    if(old != NULL && new != NULL && old != new) {
        if(old->state != TERMINATED) {
            old->state = READY;
            printk("Swicthing out %s - rsp %p\n", old->name, old->registers.rsp);
            BOCHS_MAGIC();
        ...
        if(current_task->state == NEW) {
            ...
            if(current_task->type == USER) {
                ...
                /* Kernel Tasks end up here */
                // printk("Continue kernel task\n");
                // BOCHS_MAGIC();
            }
        }
    }
